...
This repo root is **`digital-twin/`** (standalone). Terraform mirrors the Week 2 AWS pattern on Azure: **Blob Storage** (static site + private memory), **Linux Python 3.11 Functions** (default **`B1`** for dev; **`Y1`** needs **Dynamic VMs** quota; **`EP1`** needs **Elastic Premium** quota; disable with **`enable_linux_function_app = false`** when App Service quotas are **0**), **Azure OpenAI** account with model deployment **off by default** (**`enable_openai_model_deployment`** — avoids **715** on restricted subs; enable when the account can deploy **gpt-4o**). For CI when Functions and model deploy both fail, set **`PARTIAL_INFRA=true`** (storage + OpenAI account only). Optional **Azure Front Door** (`var.enable_azure_front_door`).
